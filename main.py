import streamlit as st
import os
from dotenv import load_dotenv
from PyPDF2 import PdfReader

from langchain.text_splitter import CharacterTextSplitter
from langchain_community.embeddings import HuggingFaceEmbeddings
from langchain_community.vectorstores import FAISS
from langchain_groq import ChatGroq

from langchain.chains import ConversationalRetrievalChain
from langchain.memory import ConversationBufferMemory

# Load GROQ API key
load_dotenv()
GROQ_API_KEY = os.getenv("GROQ_API_KEY")

# PDF Text Extraction
def extract_text_from_pdf(pdf_file):
    reader = PdfReader(pdf_file)
    text = ""
    for page in reader.pages:
        text += page.extract_text()
    return text

# Embedding and Vector Store
def create_vector_store(text):
    text_splitter = CharacterTextSplitter(separator="\n", chunk_size=1000, chunk_overlap=200)
    chunks = text_splitter.split_text(text)
    embeddings = HuggingFaceEmbeddings()
    vectorstore = FAISS.from_texts(chunks, embedding=embeddings)
    return vectorstore

# Create Conversational QA Chain with Memory
def create_qa_chain_with_memory(vectorstore):
    llm = ChatGroq(model="llama3-70b-8192", api_key=GROQ_API_KEY)
    memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True)
    qa_chain = ConversationalRetrievalChain.from_llm(
        llm=llm,
        retriever=vectorstore.as_retriever(),
        memory=memory
    )
    return qa_chain, memory

# Streamlit UI
st.set_page_config(page_title="Data Science Interview Q&A Bot")
st.title("💬 Data Science Interview Prep Assistant")

uploaded_pdf = st.file_uploader("Upload your PDF with Interview Q&A", type="pdf")

if uploaded_pdf:
    with st.spinner("Processing PDF and generating knowledge base..."):
        pdf_text = extract_text_from_pdf(uploaded_pdf)
        vectorstore = create_vector_store(pdf_text)
        qa_chain, memory = create_qa_chain_with_memory(vectorstore)

    st.success("Document loaded successfully. Ask your questions below!")

    if "chat_history" not in st.session_state:
        st.session_state.chat_history = []
        st.session_state.qa_chain = qa_chain
        st.session_state.memory = memory

    # Clear history button
    if st.button("🗑️ Clear Chat History"):
        st.session_state.chat_history = []
        st.session_state.memory.clear()

    user_question = st.text_input("Ask a question about your document:")

    if user_question:
        with st.spinner("Thinking..."):
            result = st.session_state.qa_chain.invoke({"question": user_question})
            answer = result["answer"]
            st.session_state.chat_history.append(("You", user_question))
            st.session_state.chat_history.append(("Bot", answer))
            st.write(answer)

    # Display chat history
    if st.session_state.chat_history:
        st.subheader("Chat History")
        for speaker, msg in st.session_state.chat_history:
            st.write(f"**{speaker}:** {msg}")

